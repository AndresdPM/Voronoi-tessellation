...
def _accretion(x, y, signal, targetSN, quiet, sn_func):
    """
...
def _cvt_equal_mass(x, y, signal, xnode, ynode, quiet, wvt, sn_func):
    """
...
def _compute_useful_bin_quantities(x, y, signal, xnode, ynode, scale, sn_func):
    """
...
def voronoi_2d_binning(data, targetSN, cvt=True,
                         quiet=True, sn_func=None, wvt=True):
    ...
    if not (x.size == y.size == signal.size):
    ...
    print('Bin-accretion...')
    classe = _accretion(x, y, signal, targetSN, quiet, sn_func)
    xNode, yNode = _reassign_bad_bins(classe, x, y)
    if cvt:
        xNode, yNode, scale, it = _cvt_equal_mass(x, y, signal, xNode, yNode, quiet, wvt, sn_func)
    else:
        scale = 1.
    classe, xBar, yBar, sn, area = _compute_useful_bin_quantities(x, y, signal, xNode, yNode, scale, sn_func)
    w = area == 1
