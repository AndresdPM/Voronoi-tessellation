...
def voronoi_stats(table, coo, name_vor_bin, n_stars = 50):
   """
...
def voronoi_stats_parallel(table, voronoi_vars, voronoi_coos, voronoi_names, voronoi_nstars, use_parallel = False):
   """
   ...
   for voronoi_coo, voronoi_name, voronoi_nstar in zip(voronoi_coos, voronoi_names, voronoi_nstars):
      args.append((table.loc[:,voronoi_vars], voronoi_coo, voronoi_name, voronoi_nstar))
   ...
   tables_voronoi = [results[0] for results in Y]
   cells_voronoi = [results[1] for results in Y]
   
   return tables_voronoi, cells_voronoi
...
if __name__=="__main__":
   ...
   voronoi_tables, voronoi_cells = voronoi_stats_parallel(table, ['x', 'y', 'z', 'z_error' ], [['x','y']], ['voronoi_cell'], [100])
   
   table['voronoi_cell'] = voronoi_cells[0]
   
   table = table.merge(voronoi_tables[0], on = ['voronoi_cell'], how = 'left')
   ...
   ax3.scatter(table.x, table.y, c = table.mean_z, alpha = 0.75, vmin = signal.min(), vmax = signal.max())
   ax3.annotate('Recovered', xy=(table.x.min(), table.y.min()), xycoords='data', color = 'k')

   ax4.scatter(table.x, table.y, c = table.voronoi_cell, alpha = 0.75, )
   ax4.annotate('Voronoi cells', xy=(table.x.min(), table.y.min()), xycoords='data', color = 'k')
